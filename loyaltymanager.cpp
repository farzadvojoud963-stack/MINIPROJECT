...
bool manager::addpointwithadmin(int id, int newpoint, string why, string type)
{
    int currentpoint = dao.getuserpoints(id);
    dao.updateuserpoint(id, newpoint);

   

    return dao.addpointhistory(id, newpoint - currentpoint, why, type);
}
